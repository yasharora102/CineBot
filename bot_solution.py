...
def greet(message):
  ...
  unique_filename = 'movie' + str(uuid.uuid1()) + ".csv"
  botRunning = True
  bot.reply_to(
    message,
...
def getMovie(message):
  ...
  with open('movie_data/' + unique_filename, 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    ...
    for i in lst:
      ...
      if 'Error' in parsed_request:
        ...
        listing = [
          parsed_request["Title"], parsed_request["Year"],
        ...
        reply = "Movie found!"
        caption = "Movie Name: " + parsed_request[
          "Title"] + "\n" + "Year: " + parsed_request[
            "Year"] + "\n" + "Released: " + parsed_request[
...
def export_file(message):
  ...
  doc = open('movie_data/' + unique_filename, 'rb')
